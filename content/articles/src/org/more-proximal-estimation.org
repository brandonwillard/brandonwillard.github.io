...
#+FILETAGS: :theano:statistics:proximal methods:optimization:symbolic computation:python:
...
#+HTML: <div class="definition" markdown="" title-name="Proximal Operator">
\begin{equation}
  \prox_{\phi}(x) =
  \argmin_{z} \left\{
...
proximal problem.
#+NAME: example_theano_graph_step
#+BEGIN_SRC python :results none
from theano import clone as tt_clone
...
def prox_gradient_step(loss, beta_tt, prox_func,
    ...
    TODO: Consider using `FromFunctionOp`.
...
#+NAME: cvxopt_run
#+BEGIN_SRC python :results hidden
...
#+BIBLIOGRAPHY: ../tex/more-proximal-estimation.bib
