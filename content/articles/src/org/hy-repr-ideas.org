...
(let* ((org-env (org-export-get-environment))
       (bib-val (plist-get org-env ':bibliography))
       (bib-loc (and bib-val
                     (apply #'f-join
                            (cdr (f-split (car bib-val))))))
       (bib-str (if bib-loc
                    (format "bibliography:\n- '%s'" bib-loc)
                  "")))
  (print (format (s-join "\n"
...
* Introduction
In the past few months, I've been working on [[https://github.com/hylang/hy][Hy]] a lot.  It's been great
for translating symbolic computation ideas originating in the Lisp community
or simply performing the generic meta-programming inherent to the subject.
