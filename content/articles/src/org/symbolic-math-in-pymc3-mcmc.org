...
# TODO: Limit this replacement to cases in which `C_lvar` is Cauchy and/or has
# an expectation ~0.

We have to make sure that the optimizer doesn't get caught in an endless $1 \to 1 \cdot 1$
loop.  To do this, in Listing [[kanren-normal-rescale-guard]], we set up a naive
constraint that projects the current miniKanren state (a src_path{dict} of logic
variable replacements) and checks that the unified value of src_python{C_lvar}
is never ~1~.
...
Furthermore, what makes a robust sampler that can handle scenarios like these?
Surprisingly, transforms exactly like the one we've implemented!  The more
context available to a sampler, the more opportunity it has to make a smarter
choice for its sampling method.  Those choices are fundamentally limited
when the context only consists of a distribution and its parameters.

The rewrite rules we're considering have no such limitation.  They have the
*entire* model to work with and--as a result--are not limited to
distribution-specific sampling bottlenecks.
...
#+HEADER: :post org_fig_wrap(data=*this*, options="[keepaspectratio]", placement="[p!]", caption="Posterior predictive counts of paid sticker violations for the Binomial model in \\eqref{eq:binom_linear_effects}.")
...
#+CAPTION: Posterior predictive counts of paid sticker violations for the Binomial model in \eqref{eq:binom_linear_effects}.
