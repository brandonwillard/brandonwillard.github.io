    ...
    def process(self, output, pstate):
        ...
        try:
            ...
            if self.latex or getattr(pstate, 'latex', False):
                ...
                dist_format += ', \quad \\mathbb{{%s}}' % sspace_char
                shapes_r = ' \\times '.join(pprinter.process(s, pstate)
                                            for s in output.shape)
                if shapes_r:
                    dist_format += '^{{%s}}' % shapes_r
            else:
                dist_format = "{} ~ {}({})"
                dist_format += ', %s' % sspace_char
                shapes_r = ' x '.join(pprinter.process(s, pstate)
                                      for s in output.shape)
                if shapes_r:
                    dist_format += '**(%s)' % shapes_r
            ...
            dist_params_r = dist_format.format(
                out_name, op_name,
...
assert tt_pprint(tt_normalrv_name_expr) == "X ~ N(\mu, \sigma), R**(A_u[0] x A_u[1])\n(b * X)"
assert tt_pprint(tt_normalrv_noname_expr) == "A ~ N(\mu, \sigma), R\n(b * A)"
assert tt_pprint(tt_2_normalrv_noname_expr) == "A ~ N(\mu_2, \sigma_2), R\nB ~ N(\mu, \sigma), R\n(A * ((b * B) + c))"
#+END_SRC
...
\begin{equation*}
\begin{aligned}
Z &\sim \text{U}\left(l_0, l_1\right), \quad \mathbb{R}
\\
X &\sim \text{N}\left(Z, \sigma_1\right), \quad \mathbb{R}
\\
Y &\sim \text{N}\left(\mu, (|X| \circ \left[\begin{matrix}1 & 0\\0 & 2\end{matrix}\right])\right), \quad \mathbb{R}^{A_u[0]}
\end{aligned}
\\
(X \circ ((b \circ Y) + c))
\end{equation*}
